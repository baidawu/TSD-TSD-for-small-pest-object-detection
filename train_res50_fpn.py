...
def main(parser_data):
    ...
    data_transform = {
        "train": transforms.Compose([transforms.ToTensor(),  # 将image转换为tensor
                                     transforms.RandomHorizontalFlip(0.5)]), # 预处理：水平翻转图片
        "val": transforms.Compose([transforms.ToTensor()])
    }

    VOC_root = parser_data.data_path
    # check voc root
    if os.path.exists(os.path.join(VOC_root, "Pest24")) is False:
        raise FileNotFoundError("Pest24 dose not in path:'{}'.".format(VOC_root))

    # load train data set
    # VOCdevkit -> VOC2012 -> ImageSets -> Main -> train.txt
    train_dataset = VOCDataSet(VOC_root, "2012", data_transform["train"], "train.txt") # 读取数据集
    train_sampler = None

    # batch_size:一次训练图片的数量，每批处理样本的个数
    ...
    model = create_model(num_classes=parser_data.num_classes + 1) # 加载模型
    # print(model)
